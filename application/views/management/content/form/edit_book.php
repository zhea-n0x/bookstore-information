  ...
  <div class="sidebar" data-color="blue">
      ...
      <div class="sidebar-wrapper" id="sidebar-wrapper">
        <ul class="nav">
          ...
          <li>
            <a href="#book" data-toggle="collapse" class="text-dark">
          ...
          <div class="col-md-6">
            <div class="card">
              <div class="card-header">
                ...
                <small class="text-small" style="margin-top: -100px;">silahkan rubah data <i>buku</i> dengan data valid pada <b>kolom</b> yang tersedia</small>
              </div>
              <div class="card-body">
                <form action="<?= base_url('control_data/updatebook');?>" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="ident" value="<?= $book['book_id'];?>" name="id">
                        <small class="form-text text-muted">nomor identifikasi tergenerate secara otomatis</small>
                    </div>
                    <div class="form-group">
                        <label for="lokasi">Judul Buku</label>
                        <input type="text" class="form-control" id="lokasi" value="<?= $book['book_title'];?>" name="judul">
                        <small class="form-text text-muted">format judul buku sesuai format pada kolom diatas</small>
                    </div>
                    <div class="form-group">
                      <label for="harga">Harga Buku</label>
                      <input type="text" class="form-control" value="<?= $book['price'];?>" name="harga">
                      <small class="form-text text-muted">format harga Rp. 25.000</small>
                    </div>
                    <div class="form-group">
                      <label for="stock">Stok Buku</label>
                      <input type="text" class="form-control" value="<?= $book['stock_unit'];?>" name="stock">
                    </div>
                    <div class="form-group">
                      <label for="author">Penulis</label>
                      <select name="author" id="" class="form-control">
                        <?php foreach ($author as $row2){?>
                          <option value="<?= $row2['author_id'];?>"><?= $row2['author_name'];?></option>
                        <?php };?>
                      </select>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                      ...
                      <select name="publisher" id="" class="form-control">
                        <?php foreach ($publisher as $row3){?>
                          <option value="<?= $row3['publisher_id'];?>"><?= $row3['publisher_name'];?></option>
                        <?php };?>
                      </select>
                    </div>
                    <div class="form-group">
                      <label for="location">Lokasi Buku</label>
                      <select name="location" id="" class="form-control">
                        <?php foreach ($location as $row4){?>
                          <option value="<?= $row4['location_id'];?>"><?= $row4['location'];?></option>
                        <?php };?>
                      </select>
                    </div>
                    <div class="d-flex justify-content-center">
                      <button type="submit" class="btn btn-primary">Submit</button>
                      <a type="button" href="<?= base_url('admin/book_management')?>" class="btn btn-secondary">Back</a>
                    </div>
                </form>
              </div>
            </div>
